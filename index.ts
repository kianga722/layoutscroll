...
const genLayout = (() => {
    ...
    const arrRand = (arr: string[]): string  => arr[Math.floor(Math.random() * arr.length)];
    // Function to randomly generate number 1 to 3
    const cardRand = (): number => Math.floor(Math.random() * 3) + 1;
    ...
    const headerWordGen = (navWords: string[]) => {
    ...
    const cardsGen = (row: HTMLElement, cardsNum: number, circle: number, border: number, isText: number, cardClass: string, cardSize: string) => {
    ...
    const hybridGen = (row: HTMLElement, border: number, cardClass: string, cardSize: string) => {
      // Add border
      if (border) {
